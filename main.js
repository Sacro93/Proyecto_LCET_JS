...
const productosCargados = async () => {
	...
	for (let producto of info) {
		let articuloNuevo = new Producto(producto.id, producto.marca, producto.modelo, producto.condicion,
			producto.segmento, producto.precio, producto.imagen);
...
function mostrarProductosCatalogo(array) {
	...
	for (let producto of array) {
		...
		nuevoProducto.innerHTML = 
							` <div class="card align-items-center border border-3 border-white" style="width: 22rem;">
                             <img src="../imagenes/productos/${producto.imagen}" class=" imagenesCard d-flex  alt="...">
							 <div class="card-body">
                          		<div class="list-group list-group-flush">

                              <h4 class="list-group-item">${producto.marca	}</h4>

                              <h5 class="list-group-item">${producto.modelo}</h5>

                              <p class="list-group-item">Condicion:  ${producto.condicion}</p>

                             <p class="list-group-item">  Valor de Cuotaparte: $${producto.precio / tickets}</p>

                              <a href="#" id= "agregarBtn${producto.id}" class="btn btn-outline-dark">Agregar</a> </div>
...
function agregarAlCarrito(producto) {
	let productoAgregadoCarrito = productosEnCarrito.find((art) => (art.id === producto.id))
...
function compraTotal(array) {
	let acumulador = 0
	acumulador = array.reduce((acc, productoCarrito) => acc + productoCarrito.precio / tickets, 0)
...
function cargarProductosCarrito(array) {
	...
	array.forEach((productoCarrito) => {
		modalBody.innerHTML += `<div class="card border-dark mb-1 container-fluid " id="productoCarrito${productoCarrito.id
			}" style="max-width: 300px;">
                      <img class="card-img-top" heigth="100px"  src="../imagenes/productos/${productoCarrito.imagen
			}" alt="${productoCarrito.nombre}">
                      <div class="card-body ">
                      <h3 class="list-group-item text-center">${productoCarrito.marca}</h3>
                      <h4 class="list-group-item text-center">${productoCarrito.modelo}</h4>
                      <p class="card-text  text-center"> Precio cuotaparte $${productoCarrito.precio / tickets
			}</p>
	...
	array.forEach((productoCarrito, indice) => {
		document
			...
			.addEventListener("click", () => {
				...
				let productoEliminar = array.find(articulo => articulo.id === productoCarrito.id)
